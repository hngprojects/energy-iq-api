...
Environment variables live in `shared/.env` for each environment. They are **never** committed to the repository and are **never** stored inside a release folder directly — each release gets a symli
