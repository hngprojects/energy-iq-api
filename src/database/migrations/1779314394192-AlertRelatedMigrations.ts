...
export class AlertRelatedMigrations1779314394192 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "delivery_processing_status" character varying(255) NOT NULL DEFAULT 'PENDING'`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "deliverable" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "delivery_status" character varying(50) NOT NULL DEFAULT 'pending'`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "delivery_channel" character varying(50)`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "quiet_hours_deferred_until" TIMESTAMP WITH TIME ZONE`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD "cooldown_expires_at" TIMESTAMP WITH TIME ZONE`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."inverters_brand_enum" AS ENUM('VICTRON', 'GROWATT', 'SUNSYNK', 'DEYE', 'SANDBOX')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."inverters_brand_enum_old" AS ENUM('VICTRON', 'GROWATT', 'SUNSYNK', 'DEYE')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP COLUMN "cooldown_expires_at"`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP COLUMN "quiet_hours_deferred_until"`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP COLUMN "delivery_channel"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP COLUMN "delivery_processing_status"`,
    );
