...
export class SandboxAdapter {
  ...
  async verifyAndGetSandboxSystem(
    ...
    if (!meRes.ok) {
      throw new BadRequestException(
        'Could not connect to your Sandbox account. Check your access token.',
      );
    }
    ...
    const instRes = await fetch(
      `${this.appCfg.sandboxApiBaseUrl}/users/${idUser}/installations`,
      {
        headers: { 'X-Authorization': `Token ${accessToken}` },
      },
    );
    ...
    if (!data.success || !data.records?.length) {
      throw new BadRequestException(
        'No installations found on your Sandbox account.',
      );
    }
  ...
  /**
   * Called on every poll cycle to fetch live metrics for a registered sandbox inverter.
   * Uses the sandbox diagnostics endpoint — returns a flat array of attribute objects
  ...
  async fetchMetrics(
    ...
    const res = await fetch(
      `${this.appCfg.sandboxApiBaseUrl}/installations/${installationId}/diagnostics`,
      { headers: { 'X-Authorization': `Token ${accessToken}` } },
    );
    ...
    if (!res.ok) {
      throw new BrandApiException(
        res.status,
        `Sandbox diagnostics request failed for installation ${installationId}`,
      );
    }
