...
export function buildAlertReportDefinition(ctx: {
  ...
  return {
    content: [
      ...buildReportHeader('Alert Report', ctx.name, ctx.period, ctx.status, ctx.dateDelivered),
      ...
      buildMetricRow('Dominant Alert Type', ctx.metrics.dominantAlertType ?? 'N/A'),
      buildMetricRow('Dominant Severity', ctx.metrics.dominantAlertSeverity ?? 'N/A'),
    ],
    styles: STYLES,
