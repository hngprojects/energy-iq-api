...
import { buildAssumptionsTable, buildMetricRow, buildReportHeader, buildSectionTitle, STYLES } from './shared';
...
export function buildGeneralReportDefinition(ctx: {
  ...
  return {
    content: [
      ...buildReportHeader('General Report', ctx.name, ctx.period, ctx.status, ctx.dateDelivered),
      ...
      buildMetricRow('Dominant Severity', metrics.dominantAlertSeverity ?? 'N/A'),

      // Cost & Savings section
      buildSectionTitle('Cost & Savings'),
      buildMetricRow('Total Cost Saved (NGN)', metrics.totalCostSavedNgn),
      buildMetricRow('Generator Cost Avoided (NGN)', metrics.generatorCostAvoidedNgn),
      buildMetricRow('Fuel Saved (Litres)', metrics.fuelSavedLitres),
      buildMetricRow('CO\u2082 Avoided (kg)', metrics.co2AvoidedKg),
      buildMetricRow('Total Energy Generated (kWh)', metrics.totalEnergyGeneratedKwh),
      buildMetricRow('Total Energy Consumed (kWh)', metrics.totalEnergyConsumedKwh),
      ...
      buildAssumptionsTable([
        ...
        ['Generator Rated Power (kW)', metrics.meta.assumedGeneratorRatedPowerKw],
