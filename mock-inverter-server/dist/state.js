...
exports.MOCK_USER_ID = void 0;
exports.tick = tick;
exports.getAllDevices = getAllDevices;
exports.getDeviceByInstallationId = getDeviceByInstallationId;
// ─── Seeded devices ──────────────────────────────────────────────────────────
const DEVICES = [
    {
 ...
 */
function solarMultiplier(hourOfDay) {
...
function timeToGo(soc, capacityKwh, netDrawKw) {
    if (netDrawKw <= 0)
        return 9999; // charging — no time-to-go
...
function tick() {
    ...
    for (const device of DEVICES) {
        ...
        device.solarPowerKw = clamp(device.panelCapacityKw * solarMult + noise(0.15), 0, device.panelCapacityKw);
        ...
        device.batteryTimeToGoMin = timeToGo(device.batterySoc, device.batteryCapacityKwh, device.acOutputPowerKw - device.solarPowerKw);
